...
import { IconPlug, IconSettings, IconLogout } from '@tabler/icons-react'
...
export default function MixIdConnection() {
  ...
  const checkConnection = async () => {
    try {
      ...
      setConnected(true)
      updateSettings({ mixIdConnected: true, syncEnabled: status.syncData })
    } catch (error) {
      setConnected(false)
    } finally {
      setLoading(false)
  ...
  const handleDisconnect = async () => {
    ...
    setConnected(false)
    setSyncStatus(null)
    updateSettings({ mixIdConnected: false, syncEnabled: false })
    notifications.show({
      title: 'Успешно',
  ...
  const handleSaveSettings = async () => {
    try {
      await mixIdApi.updateSyncPreferences(true, syncData)
      updateSettings({ syncEnabled: syncData })
      notifications.show({
        title: 'Успешно',
    ...
    <>
      <Paper p="md" withBorder>
        <Group justify="space-between">
          ...
          {connected ? (
            <Group>
              {syncStatus && (
                <Badge color={syncStatus.syncData ? 'green' : 'gray'}>Синхронизация</Badge>
              )}
      ...
      <Modal opened={settingsModalOpened} onClose={closeSettings} title="Параметры синхронизации MIX ID">
        <Stack gap="md">
          <Alert>
            <Text size="sm">
              MIX ID позволяет синхронизировать ваше зашифрованное хранилище между устройствами.
            </Text>
          </Alert>

          <Switch
            label="Синхронизировать данные"
            description="Хранилище будет синхронизироваться с сервером"
            checked={syncData}
